...
_DEFAULT_DB_PATH = Path(__file__).parent.parent.parent / "research" / "trades.db"
...
def migrate_db(db_path: Optional[Path] = None) -> bool:
    ...
    try:
        ...
        if not cursor.fetchone():
            # No trades table - nothing to migrate
            return True
        ...
        # Commit changes if any were made (ALTER TABLE auto-commits in SQLite)
        # No explicit COMMIT needed - ALTER TABLE is auto-commit

        return True

    except Exception as e:
        # Log error but don't raise - migration should be safe
...
def check_migration_status(db_path: Optional[Path] = None) -> dict:
    """Check which Phase 1 columns are present in the trades database.
    ...
    Returns:
        Dict with keys:
            ...
            - 'has_trades_table': bool - whether trades table exists
            - 'columns_present': list of column names that exist
            - 'columns_missing': list of column names that are missing
            - 'migration_needed': bool - whether migration is needed
    """
    db = db_path if db_path else _DEFAULT_DB_PATH
